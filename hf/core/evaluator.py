...
from hf.core import evaluator_utils
from hf.core import summary_utils
from hf.core import trainer_utils
from hf.core import box_util
...
KEY_SUM_RCNN_CLS_LOSS = "sum_rcnn_cls_loss"
KEY_SUM_RCNN_BIN_CLS_LOSS = "sum_rcnn_bin_cls_loss"
KEY_SUM_RCNN_REG_LOSS = "sum_rcnn_reg_loss"
KEY_SUM_RCNN_TOTAL_LOSS = "sum_rcnn_total_loss"
KEY_SUM_RCNN_CLS_ACC = "sum_rcnn_cls_accuracy"
...
class Evaluator:
    def __init__(
        ...
        self.full_model = isinstance(self.model, RcnnModel)
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        if self.full_model:
            ...
            rcnn_predictions_dir = (
                predictions_base_dir
                + "/final_predictions_and_scores/{}/{}".format(data_split, global_step)
            )
            trainer_utils.create_dir(rcnn_predictions_dir)

            if validation:
                eval_rcnn_stats = self._create_rcnn_stats_dict()
        else:
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            if self.full_model:
                rcnn_file_paths = [
                    rcnn_predictions_dir + "/{}.txt".format(sample_name)
                    for sample_name in sample_names
                ]
                if os.path.exists(rcnn_file_paths[0]):
                    continue
            else:
            ...
            if validation:
                ...
                if self.full_model:
                    # Save predictions
                    self.save_rcnn_predicted_boxes_3d_and_scores(
                        predictions, rcnn_file_paths
                    )

                    self._update_rcnn_box_cls_loc_losses(
                        eval_rcnn_stats, eval_losses, eval_total_loss, global_step
                    )
                else:
                ...
                self.get_cls_accuracy(
                    predictions,
                    eval_rcnn_stats if self.full_model else eval_rpn_stats,
                    global_step,
                )
                print(
        ...
        print(
            "Step {}: Finished evaluation, results saved to {}".format(
                global_step,
                rcnn_predictions_dir if self.full_model else prop_score_predictions_dir,
            )
        )
    ...
    def _update_rpn_losses(
        ...
        if self.full_model:
            # The full model total_loss will be the sum of Rpn and Rcnn
    ...
    def _update_rcnn_box_cls_loc_losses(
        self, eval_rcnn_stats, eval_losses, eval_total_loss, global_step
    ):
        """Helper function to calculate the evaluation average losses.
        ...
        Args:
            eval_rcnn_stats: A dictionary containing all the average
                losses.
        ...
        sum_rcnn_cls_loss = eval_rcnn_stats[KEY_SUM_RCNN_CLS_LOSS]
        sum_rcnn_bin_cls_loss = eval_rcnn_stats[KEY_SUM_RCNN_BIN_CLS_LOSS]
        sum_rcnn_reg_loss = eval_rcnn_stats[KEY_SUM_RCNN_REG_LOSS]
        sum_rcnn_total_loss = eval_rcnn_stats[KEY_SUM_RCNN_TOTAL_LOSS]

        # for the full model, we expect a total of 4 losses
        assert len(eval_losses) >= 3
        rcnn_classification_loss = eval_losses[RcnnModel.LOSS_FINAL_CLASSIFICATION]
        rcnn_bin_classification_loss = eval_losses[
            RcnnModel.LOSS_FINAL_BIN_CLASSIFICATION
        ]
        rcnn_regression_loss = eval_losses[RcnnModel.LOSS_FINAL_REGRESSION]

        sum_rcnn_cls_loss += rcnn_classification_loss
        sum_rcnn_bin_cls_loss += rcnn_bin_classification_loss
        sum_rcnn_reg_loss += rcnn_regression_loss
        sum_rcnn_total_loss += eval_total_loss

        eval_rcnn_stats.update({KEY_SUM_RCNN_CLS_LOSS: sum_rcnn_cls_loss})

        eval_rcnn_stats.update({KEY_SUM_RCNN_BIN_CLS_LOSS: sum_rcnn_bin_cls_loss})

        eval_rcnn_stats.update({KEY_SUM_RCNN_REG_LOSS: sum_rcnn_reg_loss})

        eval_rcnn_stats.update({KEY_SUM_RCNN_TOTAL_LOSS: sum_rcnn_total_loss})
        ...
        print(
            ...
            "total {:.3f}".format(
                global_step,
                rcnn_classification_loss,
                rcnn_bin_classification_loss,
    ...
    def save_prediction_stats(
        self, eval_rcnn_stats, num_valid_samples, global_step, predictions_base_dir
    ):
        """Helper function to save the RCNN loss evaluation results.

        Args:
            eval_rcnn_stats: A dictionary containing the loss sums
        ...
        sum_rcnn_cls_accuracy = eval_rcnn_stats[KEY_SUM_RCNN_CLS_ACC]

        avg_rcnn_cls_loss = sum_rcnn_cls_loss / num_valid_samples
        avg_rcnn_bin_cls_loss = sum_rcnn_bin_cls_loss / num_valid_samples
        avg_rcnn_reg_loss = sum_rcnn_reg_loss / num_valid_samples
        avg_rcnn_total_loss = sum_rcnn_total_loss / num_valid_samples

        avg_rcnn_cls_accuracy = sum_rcnn_cls_accuracy / num_valid_samples

        print(
            "Step {}: Average RCNN Losses: "
            "cls {:.5f}, "
            "bin_cls {:.5f}, "
            "reg {:.5f}, "
            "total {:.5f} ".format(
        ...
        print(
            "Step {}: Average Classification Accuracy: {:.3f} ".format(
                global_step, avg_rcnn_cls_accuracy
            )
        )

        # Append to end of file
        avg_loss_file_path = predictions_base_dir + "/rcnn_avg_losses.csv"
        with open(avg_loss_file_path, "ba") as fp:
            np.savetxt(
                fp,
                [
        ...
        avg_acc_file_path = predictions_base_dir + "/rcnn_avg_cls_acc.csv"
        with open(avg_acc_file_path, "ba") as fp:
            np.savetxt(
                fp,
                np.reshape([global_step, avg_rcnn_cls_accuracy], (1, 2)),
                fmt="%d, %.5f",
            )

    def _create_rcnn_stats_dict(self):
        """Returns a dictionary of the losses sum for averaging.
        """
        eval_rcnn_stats = dict()
        # Initialize Rcnn average losses
        eval_rcnn_stats[KEY_SUM_RCNN_CLS_LOSS] = 0
        eval_rcnn_stats[KEY_SUM_RCNN_BIN_CLS_LOSS] = 0
        eval_rcnn_stats[KEY_SUM_RCNN_REG_LOSS] = 0
        eval_rcnn_stats[KEY_SUM_RCNN_TOTAL_LOSS] = 0

        eval_rcnn_stats[KEY_SUM_RCNN_CLS_ACC] = 0

        return eval_rcnn_stats
    ...
    def get_cls_accuracy(self, predictions, eval_stats, global_step):
        """Updates the calculated accuracies for rpn and rcnn losses.

        Args:
            predictions: A dictionary containing the model outputs.
            eval_rcnn_stats: A dictionary containing all the rcnn averaged
                losses.
        ...
        if self.full_model:
            classification_pred = predictions[RcnnModel.PRED_MB_CLASSIFICATION_LOGITS]
            classification_gt = predictions[RcnnModel.PRED_MB_CLASSIFICATIONS_GT]
            classification_accuracy = self.calculate_cls_accuracy(
                classification_pred, classification_gt
            )

            sum_rcnn_cls_accuracy = eval_stats[KEY_SUM_RCNN_CLS_ACC]
            sum_rcnn_cls_accuracy += classification_accuracy
            eval_stats.update({KEY_SUM_RCNN_CLS_ACC: sum_rcnn_cls_accuracy})

            print(
                "Step {}: RCNN Classification Accuracy: {:.3f}".format(
    ...
    def save_rcnn_predicted_boxes_3d_and_scores(self, predictions, rcnn_file_paths):
        ...
        batch_pred_boxes_3d = predictions[RcnnModel.PRED_BOXES]

        # Append score and class index (object type)
        batch_pred_softmax = predictions[RcnnModel.PRED_SOFTMAX]

        batch_non_empty_box_mask = predictions[RcnnModel.PRED_NON_EMPTY_BOX_MASK]
        batch_nms_indices = predictions[RcnnModel.PRED_NMS_INDICES]
        num_boxes_before_padding = predictions[RcnnModel.PRED_NUM_BOXES_BEFORE_PADDING]
        ...
        for b in range(Batch):
            ...
            np.savetxt(rcnn_file_paths[b], predictions_and_scores, fmt="%.5f")

    def get_rcnn_predicted_box_corners_and_scores(self, predictions, box_rep):

        if box_rep in ["box_8c", "box_8co"]:
            final_pred_box_corners = predictions[RcnnModel.PRED_TOP_BOXES_8C]
        elif box_rep in ["box_4c", "box_4ca"]:
            final_pred_box_corners = predictions[RcnnModel.PRED_TOP_BOXES_4C]

        # Append score and class index (object type)
        final_pred_softmax = predictions[RcnnModel.PRED_TOP_CLASSIFICATION_SOFTMAX]
