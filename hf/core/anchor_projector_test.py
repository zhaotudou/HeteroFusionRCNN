...
class AnchorProjectorTest(unittest.TestCase):
    def test_project_to_bev(self):
        anchors = np.asarray([[1, 0, 3, 2, 0, 6], [3, 0, 3, 2, 0, 2]], dtype=np.float64)

        bev_extents = [[0, 5], [0, 10]]
        bev_extents_range = np.diff(bev_extents, axis=1)
        bev_extents_range = np.stack([bev_extents_range, bev_extents_range]).flatten()

        expected_boxes = np.asarray([[0, 4, 2, 10], [2, 6, 4, 8]], dtype=np.float64)
        expected_boxes_norm = expected_boxes / bev_extents_range

        boxes, boxes_norm = anchor_projector.project_to_bev(anchors, bev_extents)

        # Loop through cases to see errors separately
        for box, box_norm, exp_box, exp_box_norm in zip(
            boxes, boxes_norm, expected_boxes, expected_boxes_norm
        ):
    ...
    def test_project_to_bev_extents(self):
        anchors = np.asarray([[0, 0, 3, 2, 0, 6], [3, 0, 3, 2, 0, 2]], dtype=np.float64)
        ...
        expected_boxes = np.asarray(
            [[0 - (-5) - 1, 4, 0 - (-5) + 1, 10], [3 - (-5) - 1, 6, 3 - (-5) + 1, 8]],
            dtype=np.float64,
        )
        expected_boxes_norm = expected_boxes / bev_extents_range
    ...
    def test_project_to_bev_outside_extents(self):
        ...
        expected_boxes = np.asarray(
            [[0 - (-3) - 5, 9, 0 - (-3) + 5, 11]], dtype=np.float64
        )
        expected_boxes_norm = expected_boxes / bev_extents_range
    ...
    def test_project_to_bev_tensors(self):
        ...
        tf_boxes, tf_boxes_norm = anchor_projector.project_to_bev(
            tf_anchors, tf_bev_extents
        )

        np_boxes, np_boxes_norm = anchor_projector.project_to_bev(anchors, bev_extents)
    ...
    def test_3d_to_2d_point_projection(self):
        ...
        stereo_calib_p2 = np.asarray(
            [
                [7.21537700e02, 0.0, 6.09559300e02, 4.48572800e01],
                [0.0, 7.21537700e02, 1.72854000e02, 2.16379100e-01],
                [0.0, 0.0, 1.0, 2.74588400e-03],
            ]
        )

        # Do projection in numpy space
        points_2d = calib_utils.project_to_image(anchor_corners, stereo_calib_p2)

        # Do projection in tensor space
        tf_anchor_corners = tf.convert_to_tensor(anchor_corners, dtype=tf.float32)
        tf_stereo_calib_p2 = tf.convert_to_tensor(stereo_calib_p2, dtype=tf.float32)
        ...
        with sess.as_default():
            ...
            np.testing.assert_allclose(
                points_2d, points_2d_out, err_msg="Incorrect tensor 3D->2D projection"
            )
    ...
    def test_project_to_image_space_tensors(self):
        ...
        dataset_config.dataset_dir = tests.test_path() + "/datasets/Kitti/object"
        ...
        img_corners_tensor, img_corners_norm_tensor = anchor_projector.tf_project_to_image_space(
            tf_anchors, tf_stereo_calib_p2, tf_img_shape
        )
    ...
    def test_reorder_projected_boxes(self):
        ...
        tf_corners = anchor_projector.reorder_projected_boxes(box_corner_tensor)
        sess = tf.Session()
        with sess.as_default():
