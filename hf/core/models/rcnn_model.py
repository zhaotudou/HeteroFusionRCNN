...
from hf.builders import feature_extractor_builder
from hf.core import anchor_projector
from hf.core import box_3d_encoder
from hf.core import box_8c_encoder
from hf.core import projection
from hf.core import bin_based_box3d_encoder
from hf.core import pointfly as pf
from hf.core import compute_iou
from hf.core.models import model_util

from hf.core import constants

from hf.core import model
from hf.core import losses


class RcnnModel(model.DetectionModel):
    ##############################
    ...
    ##############################
    # Mini batch (mb) cls
    PRED_MB_CLASSIFICATION_LOGITS = "rcnn_mb_classification_logits"
    PRED_MB_CLASSIFICATIONS_GT = "rcnn_mb_classifications_gt"
    PRED_MB_CLASSIFICATION_MASK = "rcnn_mb_classification_mask"

    # Mini batch (mb) cls-reg
    PRED_MB_CLS = "rcnn_mb_cls"
    PRED_MB_REG = "rcnn_mb_reg"
    PRED_MB_CLS_GT = "rcnn_mb_cls_gt"
    PRED_MB_REG_GT = "rcnn_mb_reg_gt"
    PRED_MB_POS_REG_MASK = "rcnn_mb_pos_reg_mask"

    # predictions & BEV NMS
    PRED_BOXES = "rcnn_prediction_boxes"
    PRED_SOFTMAX = "rcnn_prediction_softmax"
    PRED_NON_EMPTY_BOX_MASK = "rcnn_prediction_non_empty_box_mask"
    PRED_NMS_INDICES = "rcnn_prediction_nms_indices"
    PRED_NUM_BOXES_BEFORE_PADDING = "rcnn_prediction_num_boxes_before_padding"

    ##############################
    # Keys for Loss
    ##############################
    LOSS_FINAL_CLASSIFICATION = "rcnn_classification_loss"
    LOSS_FINAL_BIN_CLASSIFICATION = "rcnn_bin_classification_loss"
    LOSS_FINAL_REGRESSION = "rcnn_regression_loss"

    # (for debugging)
    LOSS_FINAL_ORIENTATION = "rcnn_orientation_loss"
    LOSS_FINAL_LOCALIZATION = "rcnn_localization_loss"
    ...
    def __init__(self, model_config, train_val_test, dataset, batch_size=1):
        ...
        super(RcnnModel, self).__init__(model_config)
        ...
        # RCNN config
        rcnn_config = self._config.rcnn_config
        self._use_intensity_feature = rcnn_config.rcnn_use_intensity_feature
        self._fusion_method = rcnn_config.rcnn_fusion_method
        self._proposal_roi_crop_size = rcnn_config.rcnn_proposal_roi_crop_size
        self._proposal_roi_img_crop_size = [
            rcnn_config.rcnn_proposal_roi_img_crop_size
        ] * 2
        self._nms_size = rcnn_config.rcnn_nms_size
        self._nms_iou_thresh = rcnn_config.rcnn_nms_iou_thresh
        self._path_drop_probabilities = self._config.path_drop_probabilities

        self.Ss = np.asarray(rcnn_config.rcnn_xz_search_range)
        self.DELTAs = np.asarray(rcnn_config.rcnn_xz_bin_len)
        self.NUM_BIN_X = int(2 * self.Ss[0] / self.DELTAs[0])
        self.NUM_BIN_Z = self.NUM_BIN_X

        self.R = rcnn_config.rcnn_theta_search_range * np.pi
        self.DELTA_THETA = rcnn_config.rcnn_theta_bin_len * np.pi / 180
        self.NUM_BIN_THETA = int(2 * self.R / self.DELTA_THETA)

        self._pooling_context_length = rcnn_config.rcnn_pooling_context_length

        # Feature Extractor Nets
        self._pc_feature_extractor = feature_extractor_builder.get_extractor(
            self._config.layers_config.rcnn_config.pc_feature_extractor
        )
        self._img_feature_extractor = feature_extractor_builder.get_extractor(
            self._config.layers_config.rcnn_config.img_feature_extractor
        )
    ...
    def build(self, **kwargs):
        ...
        # ROI Pooling
        with tf.variable_scope("rcnn_roi_pooling"):
        ...
        with tf.variable_scope("local_spatial_feature"):
            ...
            with tf.variable_scope("mlp"):
                ...
                layers_config = self._config.layers_config.rcnn_config.mlp
        ...
        with tf.variable_scope("fts_fuse"):
            ...
            if not (
                ...
                with tf.variable_scope("rcnn_path_drop"):
        ...
        with tf.variable_scope("classification_confidence"):
            ...
            fc_layers = [fuse_rois]
            layers_config = self._config.layers_config.rcnn_config.fc_layer
            for layer_idx, layer_param in enumerate(layers_config):
                C = layer_param.C
                dropout_rate = layer_param.dropout_rate
                fc = pf.dense(
                ...
                with tf.variable_scope("bev_nms"):
                    ...
                    def sb_nms_fn(args):
                        ...
                        sb_nms_indices = sb_nms_indices[
                            : tf.minimum(self._nms_size, tf.shape(sb_nms_indices)[0])
                        ]
