...
class RcnnModelTest(tf.test.TestCase):
    ...
    def test_rcnn_loss(self):
        ...
        predictions = rcnn_model.build()
        loss, total_loss = rcnn_model.loss(predictions)
        feed_dict = rcnn_model.create_feed_dict()
    ...
    def test_canonical_transform(self):
        pts = np.array(
            [[[2.0, 0.0, 2.0], [2.0, 0.0, 3.0]], [[3.5, 0.0, 0.5], [4.0, 0.0, 1.0]]]
        )
        boxes_3d = np.array(
            [
                [2.0, 0.0, 2.0, 1.414, 1.414, 1.0, -np.pi / 4],
        ...
        pts_ct = RcnnModel._canonical_transform(pts_tensor, boxes_3d_tensor)
        """
        expceted output:
    ...
    def test_rcnn_loss_correct_class_mask(self):
        ...
        exp_correct_cls_mask = np.array([False, False, True, True, True, False, False])

        # Expected correct classifications with background removed
        exp_positive_cls_mask = np.array(
        ...
        exp_final_regression_loss = np.array(
            [0.0, 0.0, 0.0, 2.04413271, 0.0, 0.0, 0.0], dtype=np.float32
        )
        exp_num_positives = 1

        classifications_tensor = tf.convert_to_tensor(classifications, dtype=tf.float32)

        classifications_gt_tensor = tf.convert_to_tensor(
            classifications_gt, dtype=tf.float32
        )

        regression_tensor = tf.convert_to_tensor(regressions, dtype=tf.float32)

        classification_argmax = tf.argmax(classifications_tensor, axis=1)
        class_indices_gt = tf.argmax(classifications_gt_tensor, axis=1)

        correct_classifications_mask = tf.equal(classification_argmax, class_indices_gt)
        ...
        pos_regressions = regression_tensor * pos_classification_floats
        num_positives = tf.reduce_sum(pos_classification_floats)

        with self.test_session() as sess:
            # Check correct and positive classification masks
            correct_cls_mask_out, positive_cls_mask_out = sess.run(
                [correct_classifications_mask, pos_classification_mask]
            )
            self.assertAllEqual(correct_cls_mask_out, exp_correct_cls_mask)
            self.assertAllEqual(positive_cls_mask_out, exp_positive_cls_mask)

            # Check output after masks are applied
            regression_loss_out, num_positives_out = sess.run(
                [pos_regressions, num_positives]
            )
            self.assertAllEqual(regression_loss_out, exp_final_regression_loss)
            self.assertAllEqual(num_positives_out, exp_num_positives)

    def test_bool_pos_mask(self):
        # Check that applying a boolean mask gives the same output as
        # multiplying with a float mask

        pos_classification_mask = np.asarray([True, False, True], dtype=np.bool)
        ...
        angle_vectors_gt = np.asarray(
            [[1, 0], [0.8268, 0.5625], [0, 1]], dtype=np.float32
        )

        # Convert to tensors
        tf_pos_classification_floats = tf.cast(
            pos_classification_mask, dtype=tf.float32
        )
        tf_offsets = tf.convert_to_tensor(offsets, dtype=tf.float32)
        tf_offsets_gt = tf.convert_to_tensor(offsets_gt, dtype=tf.float32)
        tf_angle_vectors = tf.convert_to_tensor(angle_vectors, dtype=tf.float32)
        tf_angle_vectors_gt = tf.convert_to_tensor(angle_vectors_gt, dtype=tf.float32)

        reg_loss = losses.WeightedSmoothL1Loss()
        anchorwise_loc_loss = reg_loss(tf_offsets, tf_offsets_gt, weight=1.0)
        anchorwise_ang_loss = reg_loss(
            tf_angle_vectors, tf_angle_vectors_gt, weight=1.0
        )

        # Masking by multiplying with mask floats
        anchorwise_combined_reg_loss = (
            anchorwise_loc_loss + anchorwise_ang_loss
        ...
        with self.test_session() as sess:
            ...
            anchorwise_combined_reg_loss_out = sess.run(anchorwise_combined_reg_loss)

            # Masked with tf.boolean_mask
            pos_loc_loss_out, pos_ang_loss_out, combined_reg_loss = sess.run(
                [pos_localization_loss, pos_orientation_loss, combined_reg_loss]
            )

            pos_classification_floats_out = sess.run(tf_pos_classification_floats)

            expected_pos_loc_loss = np.sum(
                anchorwise_loc_loss_out * pos_classification_floats_out
            )
            expected_pos_ang_loss = np.sum(
                anchorwise_ang_loss * pos_classification_floats_out
            )
            expected_combined_reg_loss = expected_pos_loc_loss + expected_pos_ang_loss
            ...
            np.testing.assert_almost_equal(
                combined_reg_loss, expected_combined_reg_loss
            )
