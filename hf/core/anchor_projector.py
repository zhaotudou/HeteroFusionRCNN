...
def project_to_image_space(anchors, stereo_calib_p2, image_shape):
    ...
    if anchors.shape[1] != 6:
        raise ValueError(
            "Invalid shape for anchors {}, should be " "(N, 6)".format(anchors.shape[1])
        )
    ...
    dim_x_half = dim_x / 2.0
    dim_z_half = dim_z / 2.0
    ...
    y_corners = np.array(
        [y, y, y, y, y - dim_y, y - dim_y, y - dim_y, y - dim_y]
    ).T.reshape(1, -1)
    ...
    box_corners = np.vstack(
        [i_axis_min_points, j_axis_min_points, i_axis_max_points, j_axis_max_points]
    ).T
    ...
    image_shape_tiled = [image_shape_w, image_shape_h, image_shape_w, image_shape_h]

    box_corners_norm = box_corners / image_shape_tiled

    return (
        np.array(box_corners, dtype=np.float32),
...
def tf_project_to_image_space(anchors, stereo_calib_p2, image_shape):
    ...
    y_corners = tf.reshape(
        tf.transpose(
            tf.stack([y, y, y, y, y - dim_y, y - dim_y, y - dim_y, y - dim_y])
        ),
        (1, -1),
    )

    z_corners = tf.reshape(
        tf.transpose(
            tf.stack(
                [
                    z + dim_z_half,
    ...
    # Get the min and maxes of image coordinates
    i_axis_min_points = tf.reduce_min(tf.reshape(pts_2d[0, :], (-1, 8)), axis=1)
    j_axis_min_points = tf.reduce_min(tf.reshape(pts_2d[1, :], (-1, 8)), axis=1)

    i_axis_max_points = tf.reduce_max(tf.reshape(pts_2d[0, :], (-1, 8)), axis=1)
    j_axis_max_points = tf.reduce_max(tf.reshape(pts_2d[1, :], (-1, 8)), axis=1)
    ...
    image_shape_tiled = tf.stack(
        [image_shape_w, image_shape_h, image_shape_w, image_shape_h], axis=0
    )
...
def reorder_projected_boxes(box_corners):
    ...
    Returns:
        box_corners_reordered: tensor image corners in the format
            N x [y1, x1, y2, x2]
    """
    boxes_reordered = tf.stack(
        [box_corners[:, 1], box_corners[:, 0], box_corners[:, 3], box_corners[:, 2]],
        axis=1,
    )
...
def project_to_image_tensor(points_3d, cam_p2_matrix):
    ...
    stacked_points_2d = tf.stack([points_2d_c1, points_2d_c2], axis=0)
