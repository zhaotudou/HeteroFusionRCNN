...
def save_predictions_in_kitti_format(
    ...
    final_predictions_root_dir = (
        predictions_root_dir + "/final_predictions_and_scores/" + dataset.data_split
    )

    final_predictions_dir = final_predictions_root_dir + "/" + str(global_step)
    ...
    for sample_idx in range(num_samples):
        ...
        kitti_predictions_3d_file_path = (
            kitti_predictions_3d_dir + "/" + prediction_file
        )
        ...
        kitti_empty_1 = -1 * np.ones((len(kitti_predictions), 2), dtype=np.int32)
        ...
        # Save to text files
        np.savetxt(
            kitti_predictions_3d_file_path, kitti_text_3d, newline="\r\n", fmt="%s"
        )
...
def set_up_summary_writer(model_config, sess):
    ...
    summary_merged = summary_utils.summaries_to_keep(
        summaries, global_summaries, histograms=True, input_imgs=False, input_pcs=False
    )
...
def print_inference_time_statistics(total_feed_dict_time, total_inference_time):
...
def copy_kitti_native_code(checkpoint_name):
    ...
    if not os.path.exists(kitti_native_code_copy):
        ...
        original_kitti_native_code = (
            hf.top_dir() + "/scripts/offline_eval/kitti_native_eval/"
        )

        predictions_dir = (
            hf_root_dir + "/data/outputs/" + checkpoint_name + "/predictions/"
        )
        ...
        script_folder = predictions_dir + "/kitti_native_eval/"
    ...
    results_dir = hf.top_dir() + "/scripts/offline_eval/results"
    results_05_dir = hf.top_dir() + "/scripts/offline_eval/results_05_iou"
...
def run_kitti_native_script(checkpoint_name, score_threshold, global_step):
    ...
    make_script = eval_script_dir + "/kitti_native_eval/run_eval.sh"
    script_folder = eval_script_dir + "/kitti_native_eval/"
...
def run_kitti_native_script_with_05_iou(checkpoint_name, score_threshold, global_step):
    ...
    make_script = eval_script_dir + "/kitti_native_eval/run_eval_05_iou.sh"
    script_folder = eval_script_dir + "/kitti_native_eval/"

    results_dir = hf.top_dir() + "/scripts/offline_eval/results_05_iou/"
