...
class Box8cEncoderTest(unittest.TestCase):
    def test_box_3d_to_box_8co(self):
        ...
        gt_box_3d = np.asarray(
            [-0.69, 1.69, 25.01, 3.2, 1.66, 1.61, -1.59], dtype=np.float32
        )
        # Sample box in anchor format
        anchors = np.asarray([[-0.59, 1.90, 25.01, 3.2, 1.61, 1.66]], dtype=np.float32)
        # Convert the anchor to box3D format
        anchor_box_3d = box_3d_encoder.anchors_to_box_3d(anchors, fix_lw=True)
    ...
    def test_box_3d_tensor_to_box_8co(self):
        ...
        # convert to 8 corners
        anchor_box_corner_tensor = box_8c_encoder.tf_box_3d_to_box_8co(
            anchors_box3d_tensor
        )
        ...
        np.testing.assert_almost_equal(
            exp_anchor_box_8co[0],
            anchor_box_corner_out[0],
            decimal=2,
            err_msg="Anchor tensor corner encoding mismatch",
        )

        np.testing.assert_almost_equal(
            exp_anchor_box_8co[1],
            anchor_box_corner_out[1],
            decimal=2,
            err_msg="Anchor tensor corner encoding mismatch",
        )
    ...
    def test_align_box_8co(self):
        ...
        irregular_box_8co = np.asarray(
            [
                [
                    [6.0, 6.0, 3.0, 4.0, 5.0, 5.0, 3.0, 2.0],
        ...
        irregular_box_8co_tensor = tf.convert_to_tensor(
            irregular_box_8co, dtype=tf.float32
        )

        aligned_box_8co = box_8c_encoder.align_boxes_8c(irregular_box_8co_tensor)
        ...
        np.testing.assert_array_equal(
            exp_box_8co, aligned_box_8co_out[0], err_msg="Wrong aligned corners."
        )
    ...
    def test_box3d_to_box8c_and_back(self):
        ...
        boxes_3d = np.asarray(
            [
                [-0.59, 1.9, 25.01, 3.2, 1.61, 1.66, 0.0],
                [-0.69, 1.69, 25.01, 3.2, 1.66, 1.61, -1.59],
            ],
            dtype=np.float32,
        )
        ...
        np.testing.assert_almost_equal(boxes_3d[0], boxes_3d_out[0], decimal=2)
        np.testing.assert_almost_equal(boxes_3d[1], boxes_3d_out[1], decimal=2)
    ...
    def test_box_8co_and_box3d(self):
        ...
        box_c8_tensor = tf.convert_to_tensor(box_8co, dtype=tf.float32)
        ...
        np.testing.assert_almost_equal(box_3d_out[0], exp_box_3d, decimal=2)
    ...
    def test_tf_box_8co_diagonal_length(self):
        ...
        boxes_c8_tensor = box_8c_encoder.tf_box_3d_to_box_8co(boxes_3d_tensor)

        boxes_8c_diagonals = box_8c_encoder.tf_box_8c_diagonal_length(boxes_c8_tensor)
        boxes_3d_diagonals = box_3d_encoder.tf_box_3d_diagonal_length(boxes_3d_tensor)
    ...
    def test_box_8co_to_offset(self):
        ...
        boxes_3d_gt = np.asarray(
            [[-0.69, 1.69, 25.01, 3.2, 1.62, 1.61, -1.59]], dtype=np.float32
        )

        exp_normalized_offsets = np.asarray(
            [
                [
                    [-0.6434, -0.233, 0.592, 0.182, -0.643, -0.233, 0.592, 0.182],
                    [
                        -0.053,
                        -0.0531,
                        -0.0531,
                    ...
                    [0.197, 0.613, -0.197, -0.613, 0.197, 0.613, -0.197, -0.613],
                ]
            ]
        )
        ...
        boxes_3d_gt_tensor = tf.convert_to_tensor(boxes_3d_gt, dtype=tf.float32)

        boxes_8c = box_8c_encoder.tf_box_3d_to_box_8co(boxes_3d_tensor)

        boxes_8c_gt = box_8c_encoder.tf_box_3d_to_box_8co(boxes_3d_gt_tensor)

        normalized_offsets = box_8c_encoder.tf_box_8c_to_offsets(boxes_8c, boxes_8c_gt)
    ...
    def test_box_offset_to_8c(self):
        ...
        boxes_3d = np.asarray(
            [
                [-0.59, 1.9, 25.01, 3.2, 1.61, 1.66, 0.0],
                [-0.49, 1.8, 25.01, 3.2, 1.61, 1.78, 0.0],
            ],
            dtype=np.float32,
        )
        boxes_3d_gt = np.asarray(
    ...
    def test_np_box_8c_conversion(self):
        ...
        boxes_3d = np.asarray(
            [[-0.59, 1.9, 25.01, 3.2, 1.61, 1.66, 3.14]], dtype=np.float32
        )
        ...
        np.testing.assert_almost_equal(
            box_8c, box_8c_gt, decimal=2, err_msg="Wrong box_8c nonordered corners."
        )
