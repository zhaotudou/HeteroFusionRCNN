...
from hf.builders import optimizer_builder
...
class TrainerUtilsTest(tf.test.TestCase):
    def setUp(self):
        ...
        test_pipeline_config_path = hf.root_dir() + "/configs/unittest_pipeline.config"

        self.model_config, self.train_config, _, dataset_config = config_builder.get_configs_from_pipeline_file(
            test_pipeline_config_path, is_training=True
        )

        # Generate dataset
        self.dataset = DatasetBuilder.build_kitti_dataset(
            DatasetBuilder.KITTI_UNITTEST, use_defaults=False, new_cfg=dataset_config
        )
    ...
    def test_load_model_weights(self):
        ...
        with tf.Graph().as_default():
            model = RpnModel(
                self.model_config, train_val_test=train_val_test, dataset=self.dataset
            )
            ...
            with tf.Session() as sess:
                ...
                self.assertGreater(
                    len(rpn_weights), 0, msg="Loaded RPN weights are empty"
                )
        ...
        with tf.Graph().as_default():
            ...
            with tf.Session() as sess:
                ...
                rcnn_vars = slim.get_model_variables()
                rcnn_weights = sess.run(rcnn_vars)

                # RCNN weights should include both RPN + RCNN weights
                self.assertGreater(
                    len(rcnn_weights),
                    len(rpn_weights),
                    msg="Expected more weights for rcnn",
                )

                # grab weights corresponding to RPN by index
                # since the model variables are ordered
                rpn_len = len(rpn_weights)
                loaded_rpn_vars = rcnn_vars[0:rpn_len]
                rpn_weights_reload = sess.run(loaded_rpn_vars)

                # Make sure the reloaded weights match the originally
                # loaded weights
                for i in range(rpn_len):
                    np.testing.assert_array_equal(rpn_weights_reload[i], rpn_weights[i])
    ...
    def test_path_drop_weights(self):
        ...
        with tf.Graph().as_default():
            ...
            model = RpnModel(
                model_config, train_val_test=train_val_test, dataset=self.dataset
            )
            ...
            with tf.Session() as sess:
                ...
                for step in range(1, self.train_config.max_iterations):
                    ...
                    if step == 1:
                        current_feature_maps = sess.run(
                            model.img_feature_maps, feed_dict=feed_dict
                        )
                        exp_feature_maps = current_feature_maps
                    train_op_loss = sess.run(train_op, feed_dict=feed_dict)
                    print("Step {}, Total Loss {:0.3f} ".format(step, train_op_loss))

                    updated_feature_maps = sess.run(
                        model.img_feature_maps, feed_dict=feed_dict
                    )
