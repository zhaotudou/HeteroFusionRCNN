...
class EvaluationTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.rotated_intersection_gt = np.array([100.0, 100.0, 0, 100.0, 28.975])
        cls.gt_iou_3d = np.array([1.0, 0.3333, 0.0, 0.0200, 0.16941967548604006])
    ...
    def test_height_metrics(self):
        h_int, h_un = evaluation.height_metrics(self.gtbb_3d, self.test_cases_3d[0:3])
    ...
    def test_get_rotated_3d_bb(self):
        ...
        x_true = np.array(
            [[32.5, 32.5, 27.5, 27.5], [55.8013, -30.8013, -55.8013, 30.8013]]
        )
        z_true = np.array([[40.0, 20, 20, 40], [3.3494, -46.6506, -3.3494, 46.6506]])
        np.testing.assert_almost_equal(x_true, x, 3)
        np.testing.assert_almost_equal(z_true, z, 3)

    def test_get_rectangular_metrics(self):
        rect_met = evaluation.get_rectangular_metrics(self.gtbb_3d, self.test_cases_3d)
        np.testing.assert_almost_equal(self.rotated_intersection_gt, rect_met, 3)

        rect_met = evaluation.get_rectangular_metrics(
            self.gtbb_3d, self.test_cases_3d[4]
        )
        np.testing.assert_almost_equal(self.rotated_intersection_gt[4], rect_met, 3)
