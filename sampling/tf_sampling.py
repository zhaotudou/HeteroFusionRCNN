...
if __name__ == "__main__":
    ...
    randomnumbers = tf.random_uniform((batch_size, 10))  # (N,8192)
    ...
    us = tf.random_uniform((batch_size, 10))
    vs = tf.random_uniform((batch_size, 10))
    ...
    test = farthest_point_sample(15, pt_sample)
    reduced_sample = gather_point(pt_sample, test)

    with tf.Session() as sess:
        pt, tt, ret = sess.run([pt_sample, test, reduced_sample])
    ...
    print("pt_sample:", pt_sample.shape, pt_sample.dtype)
    print(pt)
