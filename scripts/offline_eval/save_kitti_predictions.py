...
def main():
    ...
    _, _, _, dataset_config = config_builder_util.get_configs_from_pipeline_file(
    ...
    for step_idx in range(len(global_steps)):
        ...
        for sample_idx in range(num_samples):
            ...
            kitti_predictions_2d_file_path = (
                kitti_predictions_2d_dir + "/" + prediction_file
            )
            ...
            if not save_alphas:
                ...
                alphas = all_predictions[:, 6] - np.arctan2(
            ...
            # Empty 3D (x, y, z)
            kitti_empty_2 = -1 * np.ones((len(kitti_predictions), 3), dtype=np.int32)
            # Empty 3D (h, w, l)
            kitti_empty_3 = -1000 * np.ones((len(kitti_predictions), 3), dtype=np.int32)
            # Empty 3D (ry)
            kitti_empty_4 = -10 * np.ones((len(kitti_predictions), 1), dtype=np.int32)
