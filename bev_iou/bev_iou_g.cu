...
__device__ inline float iou_bev(const float *box_a, const float *box_b, const float s_overlap){
    // params: box_a (5) [x1, y1, x2, y2, angle]
    // params: box_b (5) [x1, y1, x2, y2, angle]
...
__global__ void boxes_iou_bev_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap, float *ans_iou){
    const int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
    const int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
    ...
    const float * cur_box_b = boxes_b + b_idx * 5;
    float s_overlap = box_overlap(cur_box_a, cur_box_b);
    float cur_iou_bev = iou_bev(cur_box_a, cur_box_b, s_overlap);
    ans_overlap[a_idx * num_b + b_idx] = s_overlap;
...
void compute_bev_overlap_gpu(const int num_a, const float* boxes_a, 
                const int num_b, const float* boxes_b, float* ans_overlap){
...
void compute_bev_iou_gpu(const int num_a, const float *boxes_a, 
    ...
    boxes_iou_bev_kernel<<<blocks, threads>>>(num_a, boxes_a, num_b, boxes_b, ans_overlap, ans_iou);
}

void oriented_nms_gpu(const float *boxes, unsigned long long * mask, int boxes_num, float nms_overlap_thresh){
