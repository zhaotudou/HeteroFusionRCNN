...
static inline Status ParseAndCheckBoxSizes(const Tensor& boxes,
  ...
  // The shape of 'boxes' is [num_boxes, 3, 8].
  if (boxes.dims() != 3) {
    return errors::InvalidArgument("boxes must be 3-D",
                                    boxes.shape().DebugString());
  }
  *num_boxes = boxes.dim_size(0);
  if (boxes.dim_size(1) != 3) {
    return errors::InvalidArgument("boxes must have 3 axis");
  }
  if (boxes.dim_size(2) != 8) {
    return errors::InvalidArgument("boxes must have 8 corners");
  }
