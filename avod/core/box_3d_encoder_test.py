...
class Box3dEncoderTest(unittest.TestCase):
    def test_box_3d_to_anchor(self):
        ...
        expected_anchors = np.asarray(
            [[1, 2, 3, 4, 6, 5], [0, 0, 0, 1, 3, 2], [0, 0, 0, 2, 3, 1]],
            dtype=np.float64,
        )
    ...
    def test_box_3d_to_anchor_180_270(self):
        box_3d = np.asarray(
            [[1, 2, 3, 4, 5, 6, np.pi], [1, 2, 3, 4, 5, 6, 3 * np.pi / 2]],
            dtype=np.float64,
        )

        expected_anchors = np.asarray(
            [[1, 2, 3, 4, 6, 5], [1, 2, 3, 5, 6, 4]], dtype=np.float64
        )
    ...
    def test_box_3d_to_anchor_rotated(self):
        ...
        box_3d = np.asarray(
            [[1, 2, 3, 4, 5, 6, np.pi * 4 / 5], [1, 2, 3, 4, 5, 6, 8 * np.pi / 5]],
            dtype=np.float64,
        )
    ...
    def test_anchor_tensor_to_box_3d(self):
        ...
        boxes_3d = box_3d_encoder.anchors_to_box_3d(anchor_tensors, fix_lw=True)
    ...
    def test_box_3d_tensor_to_anchor(self):
        ...
        boxes_3d_tensors = tf.convert_to_tensor(boxes_3d, dtype=tf.float32)
