...
def build(
    ...
    if not len(input_weights) == len(input_rois):
        raise ValueError(
            "Length of input_weights does not match length of " "input_rois"
        )
    if not len(layer_sizes) == num_layers:
        raise ValueError("Length of layer_sizes does not match num_layers")

    if fusion_type == "early":
        cls_logits, offsets, angle_vectors = _early_fusion_fc_layers(
        ...
        cls_logits, offsets, angle_vectors = _late_fusion_fc_layers(
            num_layers=num_layers,
            layer_sizes=layer_sizes,
            input_rois=input_rois,
            input_weights=input_weights,
        ...
        cls_logits, offsets, angle_vectors = _deep_fusion_fc_layers(
            num_layers=num_layers,
            layer_sizes=layer_sizes,
            input_rois=input_rois,
            input_weights=input_weights,
...
def build_output_layers(tensor_in, num_final_classes, box_rep):
    ...
    with slim.arg_scope(
        ...
        for layer_idx in range(num_layers):
            ...
            fc_layer = slim.fully_connected(
                fc_drop, layer_sizes[layer_idx], scope="fc{}".format(fc_name_idx)
            )
...
def _deep_fusion_fc_layers(
    ...
    with slim.arg_scope(
        ...
        for layer_idx in range(num_layers):
            ...
            # Apply fusion
            fusion_layer = avod_fc_layer_utils.feature_fusion(
                fusion_method, all_branches, input_weights
            )
