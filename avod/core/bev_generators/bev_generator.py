...
class BevGenerator:
    ...
    def _create_density_map(
        self, num_divisions, voxel_indices_2d, num_pts_per_voxel, norm_value
    ):

        # Create empty density map
        density_map = np.zeros((num_divisions[0], num_divisions[2]))

        # Only update pixels where voxels have num_pts values
        density_map[voxel_indices_2d[:, 0], voxel_indices_2d[:, 1]] = np.minimum(
            1.0, np.log(num_pts_per_voxel + 1) / norm_value
        )
