...
class Box4cEncoderTest(unittest.TestCase):
    def test_np_box_3d_to_box_4c(self):
        ...
        exp_box_4c_1 = np.asarray(
            [1.0, 1.0, -1.0, -1.0, 0.5, -0.5, -0.5, 0.5, 2.0, 7.0]
        )
        exp_box_4c_2 = np.asarray(
            [0.5, 0.5, -0.5, -0.5, 1.0, -1.0, -1.0, 1.0, 2.0, 7.0]
        )
    ...
    def test_np_box_3d_to_box_4c_rotated_translated(self):
        ...
        np.testing.assert_almost_equal(
            box_4c_translated, exp_box_4c_translated, decimal=3
        )
    ...
    def test_np_box_3d_to_box_4c_heights(self):
        ...
        # Expected boxes_4c
        exp_box_4c_1 = np.asarray(
            [1.0, 1.0, -1.0, -1.0, 0.5, -0.5, -0.5, 0.5, -1.0, 4.0]
        )
        exp_box_4c_2 = np.asarray(
            [1.0, 1.0, -1.0, -1.0, 0.5, -0.5, -0.5, 0.5, 0.0, 5.0]
        )
        exp_box_4c_3 = np.asarray(
            [1.0, 1.0, -1.0, -1.0, 0.5, -0.5, -0.5, 0.5, 1.0, 6.0]
        )
    ...
    def test_tf_box_3d_to_box_4c(self):
        ...
        tf_boxes_4c = box_4c_encoder.tf_box_3d_to_box_4c(tf_boxes_3d, tf_ground_plane)
        ...
        with sess.as_default():
            ...
            for box_idx in range(len(np_boxes_4c)):
                np.testing.assert_almost_equal(
                    np_boxes_4c[box_idx], tf_boxes_4c_out[box_idx], decimal=5
                )

    def test_np_box_4c_to_box_3d(self):
        box_4c_1 = np.asarray([1.0, 0.0, -1.0, 0.5, 0.5, -1.0, 0.0, 1.0, 1.0, 3.0])

        box_4c_2 = np.asarray([1.0, 0.0, -1.0, -0.5, 0.0, -1.0, 0.5, 1.0, 1.0, 3.0])
    ...
    def test_tf_box_4c_to_box_3d(self):
        ...
        np_boxes_3d = [
            box_4c_encoder.np_box_4c_to_box_3d(box_4c, np_ground_plane)
            for box_4c in np_boxes_4c
        ]

        tf_boxes_4c = tf.convert_to_tensor(np_boxes_4c, dtype=tf.float32)
        tf_ground_plane = tf.convert_to_tensor(np_ground_plane, dtype=tf.float32)

        tf_boxes_3d = box_4c_encoder.tf_box_4c_to_box_3d(tf_boxes_4c, tf_ground_plane)
        ...
        with sess.as_default():
            ...
            for box_idx in range(len(np_boxes_3d)):
                np.testing.assert_almost_equal(
                    np_boxes_3d[box_idx], tf_boxes_3d_out[box_idx], decimal=3
                )
