...
class OrientationEncoderTest(tf.test.TestCase):
    def test_tf_orientation_to_angle_vector(self):
        ...
        expected_angle_vectors = np.stack(
            [np.cos(np_orientations), np.sin(np_orientations)], axis=1
        )
        ...
        with self.test_session() as sess:
            ...
            np.testing.assert_allclose(angle_vectors_out, expected_angle_vectors)
    ...
    def test_angle_vectors_to_orientation(self):
        ...
        expected_orientations = np.arctan2(
            np_angle_vectors[:, 1], np_angle_vectors[:, 0]
        )
        ...
        with self.test_session() as sess:
            ...
            np.testing.assert_allclose(orientations_out, expected_orientations)
    ...
    def test_zeros_angle_vectors_to_orientation(self):
        ...
        # Convert to tensors and convert to orientation angles
        tf_angle_vectors = tf.convert_to_tensor(np_angle_vectors, dtype=tf.float64)
        tf_orientations = orientation_encoder.tf_angle_vector_to_orientation(
            tf_angle_vectors
        )
