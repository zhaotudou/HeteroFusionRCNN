...
class BevVgg(bev_feature_extractor.BevFeatureExtractor):
    ...
    def build(self, inputs, input_pixel_size, is_training, scope="bev_vgg"):
        ...
        with slim.arg_scope(
            self.vgg_arg_scope(weight_decay=vgg_config.l2_weight_decay)
        ):
            with tf.variable_scope(scope, "bev_vgg", [inputs]) as sc:
                ...
                with tf.variable_scope("upsampling"):
                    ...
                    feature_maps_out = tf.image.resize_bilinear(net, upsampled_shape)
    ...
    def build(
        ...
        with tf.variable_scope(scope, "bev_vgg", [inputs]) as sc:
            ...
            with slim.arg_scope(
                ...
                net = slim.conv2d(net, 4096, [9, 9], padding="VALID", scope="fc6")
                net = slim.dropout(
