...
def visualization(image_dir, index, flipped=False, display=True, fig_size=(15, 9.15)):
    ...
    if not flipped:
        # Grab image data
        img = np.array(Image.open("%s/%06d.png" % (image_dir, index)), dtype=np.uint8)
        # plot images
        ax1.imshow(img)
        ax2.imshow(img)
...
def visualize_single_plot(
    image_dir, img_idx, flipped=False, display=True, fig_size=(12.9, 3.9)
):
    """Forms the plot figure and axis for the visualization
    ...
    if not flipped:
        # Grab image data
        img = np.array(Image.open("%s/%06d.png" % (image_dir, img_idx)), dtype=np.uint8)
        # plot images
        ax.imshow(img)
        ...
        # plot images
        ax.imshow(cv2.cvtColor(image_flipped, cv2.COLOR_BGR2RGB))
...
def draw_box_3d(
    ...
    if len(corners) > 0:
        for i in range(4):
            ...
            ax.plot(
                x, y, linewidth=line_width, color=box_color, linestyle=trun_style[trc]
            )
...
def plot_3d_cube(corners, ax, c="lime"):
    ...
    ax.plot([p2[0], p3[0]], [p2[1], p3[1]], [p2[2], p3[2]], c=c)

    ax.plot([p3[0], p4[0]], [p3[1], p4[1]], [p3[2], p4[2]], c=c)

    ax.plot([p4[0], p1[0]], [p4[1], p1[1]], [p4[2], p1[2]], c=c)

    #############################
    # Top Face
    #############################
    ax.plot([p5[0], p6[0]], [p5[1], p6[1]], [p5[2], p6[2]], c=c)

    ax.plot([p6[0], p7[0]], [p6[1], p7[1]], [p6[2], p7[2]], c=c)

    ax.plot([p7[0], p8[0]], [p7[1], p8[1]], [p7[2], p8[2]], c=c)

    ax.plot([p8[0], p5[0]], [p8[1], p5[1]], [p8[2], p5[2]], c=c)
    #############################
    # Front-Back Face
    #############################
    ax.plot([p5[0], p8[0]], [p5[1], p8[1]], [p5[2], p8[2]], c=c)

    ax.plot([p8[0], p4[0]], [p8[1], p4[1]], [p8[2], p4[2]], c=c)

    ax.plot([p4[0], p1[0]], [p4[1], p1[1]], [p4[2], p1[2]], c=c)

    ax.plot([p1[0], p5[0]], [p1[1], p5[1]], [p1[2], p5[2]], c=c)
    #############################
    # Front Face
    #############################
    ax.plot([p2[0], p3[0]], [p2[1], p3[1]], [p2[2], p3[2]], c=c)

    ax.plot([p3[0], p7[0]], [p3[1], p7[1]], [p3[2], p7[2]], c=c)

    ax.plot([p7[0], p6[0]], [p7[1], p6[1]], [p7[2], p6[2]], c=c)

    ax.plot([p6[0], p2[0]], [p6[1], p2[1]], [p6[2], p2[2]], c=c)
