...
def main():
    ...
    if bev_generator == "slices":
        text_format.Merge(
            slices_config, dataset_config.kitti_utils_config.bev_generator
        )
    else:
    ...
    vis_utils.cv2_show_image(
        "Image", image, size_wh=rgb_img_size, location_xy=(img_x, 0)
    )
    ...
    vis_utils.cv2_show_image(
        "Density Map", density_map, size_wh=(img_w, img_h), location_xy=(img_x, img_y)
    )
